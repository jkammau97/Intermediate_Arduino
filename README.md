...
>In the last assignment, you used a potentiometer to control an LED.  Now you'll control a motor.  One major difference: motors can draw a lot more juice (current) than LEDs.  In fact, they draw so much current, we don't want to power them directly from the Arduino.  That might fry the Arduino.  So, we will power our motor directly from a 6 V battery pack

>For this circuit, you'll read the potentiometer value like before.  That reading will determine how much voltage you send to the transistor base (through a 220 Ω resistor) and that in turn will determine how much current runs from the 6 V battery pack through the motor.  The motor/transistor part of the circuit is below.  Notice the diode.  Diodes are one-way valves for current.  We are using one in this circuit to prevent back emf generated by the motor (take AP Physics C and you'll know what I mean) from messing with the transistor. 

>Let's wire 5 V from the Arduino to one red rail on your breadboard and GND from your Arduino to the blue rail next to it.  Then, wire +6 V from your battery pack to the other red rail and ground from your battery pack to the blue rail next to it.  Then connect the two blue ground rails.  That means everyone's ground is the same.  Once you've done that, you can wire the circuit below.  6 V straight to one side of the motor.  The other side passes through a transistor on its way to ground.  How much current is allowed through the transistor is controlled by the Arduino.  Turn the potentiometer all the way to the left and the motor is off.  Turn the potentiometer up and the motor speeds up.  Piece of cake, right?
